...
export default function EditProduct() {
  ...
  useEffect(() => {
    if (!id) return;
    api.get<{ product: any }>(`/admin/products/${id}`).then((res) => {
      const p = res.product;
      setName(p.name);
      setDescription(p.description || '');
      setHeritageStory(p.heritageStory || '');
      setSku(p.sku);
      ...
      if (p.variants?.length) setVariants(p.variants.map((v: any) => ({ ...v, key: Math.random().toString() })));
      if (p.images?.length) setImages(p.images.map((img: any) => ({ ...img, uploading: false })));
    }).catch(err => {
  ...
  const handleSubmit = async (e: React.FormEvent, publish: boolean) => {
    ...
    try {
      ...
      const res = await api.patch<{ product: { id: string } }>(`/admin/products/${id}`, payload);
      navigate('/admin/products');
    } catch (err: any) {
      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-2">Edit Product</h1>
            <p className="text-slate-500 dark:text-slate-400">Modify an existing product listing.</p>
          </div>
