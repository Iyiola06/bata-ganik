...
export default function CatalogueExport() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.get<{ products: any[] }>(`/admin/products?limit=100&category=${selectedCategory}`)
  ...
  return (
    <div className={`flex flex-col lg:flex-row min-h-[calc(100vh-64px)] ${theme === 'dark' ? 'dark bg-slate-900' : 'bg-slate-50'}`}>
      ...
      <div className="w-full lg:w-80 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 print:hidden flex flex-col gap-6 sticky top-0 h-[calc(100vh-64px)] overflow-y-auto z-10">
        ...
        <div>
          <label className="block text-sm font-bold text-slate-900 dark:text-white mb-2">Category Filter</label>
          <select 
        ...
        <div>
          ...
          <div className="flex bg-slate-100 dark:bg-slate-800 rounded-lg p-1">
            <button 
              ...
              className={`flex-1 text-sm font-medium py-2 rounded-md transition-colors ${theme === 'light' ? 'bg-white shadow text-slate-900' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`}
            >
              Light Mode
            </button>
            <button 
              onClick={() => setTheme('dark')}
              className={`flex-1 text-sm font-medium py-2 rounded-md transition-colors ${theme === 'dark' ? 'bg-slate-700 shadow text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`}
            >
              Dark Mode
            </button>
          </div>
        ...
        <button 
          onClick={handlePrint}
          disabled={!selectedCategory || loading || products.length === 0}
      ...
      {/* Preview Pane - forms the printed page */}
      <div className={`flex-1 p-8 lg:p-12 overflow-y-auto print:p-0 print:m-0 print:bg-transparent print:overflow-visible bg-white dark:bg-slate-950 text-slate-900 dark:text-white ${theme}`}>
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-slate-400 print:hidden">
            ...
            <p className="text-lg font-medium">Select a category to preview catalogue</p>
          </div>
        ) : products.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-slate-400 print:hidden">
            <span className="material-symbols-outlined text-6xl mb-4 opacity-50">inventory_2</span>
            <p className="text-lg font-medium">No products found for this category</p>
          </div>
        ) : (
                ...
                <div key={product.id} className="break-inside-avoid mb-8">
                  ...
                  {product.variants?.length > 0 && (
                    <div className="mt-auto">
                      <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1">Available Sizes (EU)</p>
                      <div className="flex flex-wrap gap-1">
                        {Array.from(new Set<string>(product.variants.map((v: any) => v.sizeEU))).sort().map(size => (
                          <span key={size} className="text-[10px] font-medium border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 px-1.5 py-0.5 rounded">
                            {size}
                          </span>
                        ))}
                      </div>
